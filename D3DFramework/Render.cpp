...
namespace D3D11Framework
{
	...
	bool Render::CreateDevice(HWND hwnd)
	{
		...
		if (!m_createdevice())
		{
			Log::Get()->Err("Не удалось создать DirectX Device, render.cpp if (!m_createdevice())");
			return false;
		}

		if (!m_createdepthstencil())
		{
			Log::Get()->Err("Не удалось создать буфер глубины, render cpp if (!m_createdepthstencil())");
			return false;
		}

		if (!m_createblendingstate())
		{
			Log::Get()->Err("Не удалось создать blending state, render.cpp if (!m_createblendingstate())");
			return false;
		}
